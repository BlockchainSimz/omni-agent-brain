...
export class Metrics {
  ...
  observe(name, value) { if (!Number.isFinite(value)) throw new Error('metric value must be finite'); const values = this.timings.get(name) || []; values.push(value); this.timings.set(name, values); }
  snapshot() { const timing = {}; for (const [name, values] of this.timings) timing[name] = { count: values.length, min: Math.min(...values), max: Math.max(...values), avg: values.reduce((a, b) => a + b, 0) / values.length }; return { counters: Object.fromEntries(this.counters), timings: timing }; }
}
