...
const server = http.createServer(async (req, res) => {
  try {
    ...
    const status = /not found|required|must include|blocked|requires|too large/i.test(error.message) ? 400 : 500;
    json(res, status, { error: status === 500 ? 'internal_error' : 'invalid_request', message: error.message });
  }
});
