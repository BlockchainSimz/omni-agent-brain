...
export class BrainStore {
  constructor() { this.memories = new Map(); this.skills = new Map(); this.audit = []; this.auditHead = 'GENESIS'; }
  ...
  remember(input) {
    ...
    this.memories.set(id, item); this.record('memory.created', id, { sourceHash: item.sourceHash, confidence: item.confidence }); return structuredClone(item);
  }
  ...
  validateMemory(id, result) {
    ...
    this.record('memory.validated', id, { passed: result.passed, confidence: item.confidence }); return structuredClone(item);
  }
  ...
  proposeSkill(input) {
    ...
    this.skills.set(id, skill); this.record('skill.proposed', id, { name: skill.name }); return structuredClone(skill);
  }
  ...
  promoteSkill(id, evaluation) {
    ...
    this.record('skill.promoted', id, { version: skill.version, score: skill.evaluation.score }); return structuredClone(skill);
  }
  ...
  rollbackSkill(id, reason) {
    ...
    this.record('skill.rollback', id, { reason: skill.rollbackReason }); return structuredClone(skill);
  }
  ...
  record(event, subjectId, data = {}) {
    const at = new Date().toISOString(); const id = crypto.randomUUID(); const payload = { id, event, subjectId, data, at, previousHash: this.auditHead }; const hash = sha256(JSON.stringify(payload));
    const entry = { ...payload, hash }; this.audit.push(entry); this.auditHead = hash;
  }
